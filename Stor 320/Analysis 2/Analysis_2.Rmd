...
Answer *(1 Point)*: Yes.It is a primary key, as each observation has a unique value in this column.

Is the variable, `Margarine` in `CHOL1` a primary key?

Answer *(1 Point)*: NO. It does not contain unique value for each row, only A and B.
...
```{r}
CHOL1b<-CHOL1a%>%pivot_longer("4":"8",names_to="Week",values_to="Response")%>%mutate_at("Week",as.numeric)
str(CHOL1b)
```
...
```{r}
CHOL2a<-CHOL2%>%pivot_wider(names_from="Statistic",values_from="Value")
print(CHOL2a)
```
...
Answer *(1 Point)*: No, it is no longer unique.

Is the variable, `Margarine` in `CHOL1b` a primary key?

Answer *(1 Point)*: No, it is also not unique

Is the variable, `Brand` in `CHOL2a` a primary key? 

Answer *(1 Point)*: Yes, it only has A and B, both unique.
...
```{r}
CHOL.COMBINED<-CHOL1b%>%left_join(CHOL2a,by=c("Margarine"="Brand"))
head(CHOL.COMBINED)
```
...
```{r}
VIOLENT2<-VIOLENT%>%anti_join(S_VS_D,by=c("State"="STATE"))
str(VIOLENT2)
```
...
VIOLENT4<-VIOLENT3%>%left_join(CENSUS,by=c("City"="Name","State"))
head(VIOLENT4)
unique(VIOLENT4$City[is.na(VIOLENT4$Total)])

```


Answer *(2 Points)*: The cities with missing crime information in VIOLENT4 is "Charlotte"  "Durham"     "Greensboro" "Toledo" 
...
```{r}
FINAL.VIOLENT<-VIOLENT4%>%left_join(STATE_ABBREV,by=c("State"))%>%left_join(ZIP,by=c("state","City"="city"))%>%select(-state)

FINAL.VIOLENT%>%filter(is.na(lat)&is.na(lon))
str(FINAL.VIOLENT)
...
Answer *(1 Points)*: Since the abbreviation for state does not include DC, so we could not find a state abbreviation name DC for state District of Columbia in VIOLENT4 so we could not find a match with DC in ZIP as we joined them based on the state abbreviation name.  

Code and Output *(1 Point)*:
```{r}
State_ABBREV1<-rbind(STATE_ABBREV,c("District of Columbia", "DC"))
FINAL.VIOLENT.FIX<-FINAL.VIOLENT%>%left_join(State_ABBREV1,by=c("State"))%>%left_join(ZIP,by=c("state","City"="city"))%>%select(-state,-lat.x,-lon.x)
str(FINAL.VIOLENT.FIX%>%filter(City=="Washington"))
```
...
Answer *(2 Points)*: https://www.worldometers.info/gdp/gdp-by-country/
...
Answer *(2 Points)*: The data I have found on the website shows the GDP performance for each country. It has variables like gross GDP, GDP per capita, population, GDP growth, Share of world GDP and Country name. Most of the data are in character forms and they have symbols redundant. 
