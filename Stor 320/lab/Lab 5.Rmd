...
```{r}
Salaries%>%filter(playerID=="trumbma01")
```
...
### Question 4
4. Draw a scatterplot with dots representing destination locations and colors of dots representing average arrival delay on US map. (4 Points)
```{r, eval=T}
delay %>%
  ggplot(aes(lon,lat)) +
    borders("state") +
...
```
Answer: It was cancelled planes
...
Answer: I think it's the problem of carriers, MQ and AA takes up about 90% of the planes not matching with flights. 