...
for (i in seq_along(x)){
  y[i] =  if(x[i] %in% c(3,10,17,24)){
              "M"
          }else if(x[i] %in% c(4,11,18,25)){"Tu"

          }else if(x[i] %in% c(5,12,19,26)){"W"

          }else if(x[i] %in% c(6,13,20,27)){"Th"
...
y.factor = factor(y,levels=c("M","Tu","W","Th","F"))
...
DATA1$DAYNAME = DAYNAME.func(DATA1$DAY) #DO NOT CHANGE
...
plot=ggplot(data=SUB.DATA) + 
      geom_line(aes(x=1:nrow(SUB.DATA),y=L101,color=as.factor(DAY))) + 
...
TRAFFICSUM.func(10) #DO NOT CHANGE
```
...
for(k in unique(DATA2$DAY)){
  id.day=which(DATA2$DAY==k)
  subdata1=DATA2[id.day,]
  id.sensor=which(names(DATA2)=="L103")
  subdata2=subdata1[[id.sensor]]
  medIQRsum=c(k,median(subdata2),IQR(subdata2))
  OUTPUT.A = rbind(OUTPUT.A,medIQRsum)
}
...
for(k in unique(DATA2$DAY)){
  i = which(unique(DATA2$DAY)==k)
  id.day=which(DATA2$DAY==k)
  subdata1=DATA2[id.day,]
  id.sensor=which(names(DATA2)=="L103")
  subdata2=subdata1[[id.sensor]]
   medIQRsum=c(k,median(subdata2),IQR(subdata2))
  OUTPUT.B[i,]= medIQRsum
}
...
Code and Output:
```{r,eval=T}
RANGE.AGG.OCC = apply(OCC,1,RANGE.func1)
RANGE.AGG.VOL = apply(VOL,1,RANGE.func1)
